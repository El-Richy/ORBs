//+------------------------------------------------------------------+
//|                           ORB_VF.mq5                            |
//|      Opening Range Breakout — Versión Final (Filtros Optimizados)|
//|      Compatible: NAS100, US500, US30, JPN225                     |
//|                                                                  |
//|  Filtros eliminados vs ORB5min_EA (por backtesting):             |
//|  - VWAP        (-rentabilidad, mismo DD)                         |
//|  - C3 vela     (sin diferencia relevante)                        |
//|  - C9 volumen  (-rentabilidad, mismo DD)                         |
//|  - C2 previo   (-rentabilidad, mismo DD)                         |
//|  - Noticias    (sin dependencias externas)                       |
//|                                                                  |
//|  Fix: filtro de spread usa SymbolInfoInteger (más preciso)       |
//+------------------------------------------------------------------+
...
input bool     UseSpreadFilter   = true;                                      // Activar filtro de spread máximo
input int      MaxSpreadPoints   = 30;                                        // Spread máximo permitido en points (30 = ~0.3 en NAS100)
//--------------------------------------------------------------------
...
input bool     UseFilterEMA      = true;                                      // Filtro EMA: cruce rápida/lenta como sesgo
//--------------------------------------------------------------------
input string   separador5c = "------- Filtro días de semana -------";        // ----------- Separador -----------
...
input bool     UseDrawdownProtection = true;                                  // Activar protección de drawdown máximo
...
//=======================================================
#define ORB_HIGH_LINE  "ORB_VF_HIGH"
#define ORB_LOW_LINE   "ORB_VF_LOW"
...
int OnInit()
  {
   ...
   if(handleATR    == INVALID_HANDLE || handleATR_TF == INVALID_HANDLE ||
     ...
     {
      Print("ERROR: Fallo al crear handles. Código: ", GetLastError());
      return INIT_FAILED;
     }
   ...
   Print("ORB_VF v1.00 iniciado en ", _Symbol, " TF=", EnumToString(_Period));
...
//+------------------------------------------------------------------+
//| Dibuja líneas ORB_HIGH y ORB_LOW en el gráfico                  |
//+------------------------------------------------------------------+
void DrawORBLines()
  {
...
//+------------------------------------------------------------------+
//| Retorna solo la fecha (inicio del día)                           |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| PASO 1 — Formación del rango ORB                                 |
//| Usa _Period — compatible con cualquier temporalidad              |
//+------------------------------------------------------------------+
void TryFormRange()
  {
...
void CheckDrawdownProtection()
  {
   ...
   if(ddPercent >= MaxDDPercent)
     {
      Print("⚠️ DD PROTECTION — DD=", DoubleToString(ddPercent, 2),
            "% >= límite=", MaxDDPercent, "%. Cerrando y desactivando EA.");
...
void ManageOpenPosition()
  {
   ...
   // ATR del timeframe actual para trailing dinámico
   double atrTFBuf[1];
   double atrTF = R * 0.5; // fallback si no hay datos
         ...
         if(newSL > currentSL)
           {
            if(trade.PositionModify(ticket, newSL, currentTP))
              { beActivated = true; Print("BE BUY activado. SL=", DoubleToString(newSL, _Digits)); }
            else
      ...
      if(UseBreakEven && !beActivated && price <= entryPrice - R)
        {
         ...
         if(currentSL == 0.0 || newSL < currentSL)
           {
            if(trade.PositionModify(ticket, newSL, currentTP))
              { beActivated = true; Print("BE SELL activado. SL=", DoubleToString(newSL, _Digits)); }
            else
      ...
      if(ticket > 0)
        {
         if(trade.PositionClose(ticket))
            Print("PASO 7 — Cerrado por tiempo: ", TimeToString(TimeCurrent(), TIME_MINUTES));
         else
...
void OnTick()
  {
   ...
   //--- PASO 3: Sesgo por EMA (el único filtro de sesgo restante)
   // Si UseFilterEMA=false, ambos bias son true → permite entradas en ambas direcciones
   bool biasAlcista = UseFilterEMA ? (emaFast > emaSlow) : true;
   bool biasBajista = UseFilterEMA ? (emaFast < emaSlow) : true;
   ...
   //--- Filtro de spread
   // FIX: usa SymbolInfoInteger (entero nativo) en vez de (ask-bid)/_Point
   // Default 30 puntos — ajustar según el spread típico del símbolo y broker
   if(UseSpreadFilter && (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) > MaxSpreadPoints) return;
   ...
   // Filtros activos: C1 (breakout), UseFilterATR (C6), UseFilterEMA (sesgo)
      ...
      if(trade.Buy(lot, _Symbol, entry, sl, tp, "ORB BUY"))
        {
         ...
         Print("BUY ejecutado. tradesToday=", tradesToday,
               " ORB_SIZE=", DoubleToString(ORB_SIZE, _Digits));
        }
      else
      ...
      if(trade.Sell(lot, _Symbol, entry, sl, tp, "ORB SELL"))
        {
         ...
         isBreakout = true; entryPrice = entry; slDistance = dist;
         beActivated = false; trailActivated = false;
         Print("SELL ejecutado. tradesToday=", tradesToday,
               " ORB_SIZE=", DoubleToString(ORB_SIZE, _Digits));
        }
      else
...
//+------------------------------------------------------------------+
//| OnTradeTransaction — reset al cerrar posición                    |
//+------------------------------------------------------------------+
